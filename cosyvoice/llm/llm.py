...
class Qwen2LM(TransformerLM):
    ...
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid):
        if hasattr(self, 'vllm'):
                ...
                while True:
                    ...
                    if self.vllm_output_queue[uuid].empty() is False:
                        ...
                        if len(out_tokens) == max_len:
                            break
                    # 优化：减少sleep时间，使用更短的间隔
                    time.sleep(0.0005)
            finally:
                # 确保无论是否发生异常，都能正确清理队列，防止内存泄漏
                with self.lock:
                    if uuid in self.vllm_output_queue:
            ...
            # 优化：预先创建单步解码的mask（1x1x1），避免每次循环都创建
            # 这在高负载环境下可以减少CPU开销
            single_step_mask = torch.ones((1, 1, 1), device=device, dtype=torch.bool)
            
            for i in range(max_len):
                seq_len = lm_input.shape[1]
                
                # 优化：只在首次需要大mask时创建，后续使用缓存的单步mask
                if seq_len == 1 and cache is not None:
                    # 单步解码，使用预创建的mask
                    masks = single_step_mask
                else:
                    # 首次解码或需要完整mask
                    masks = torch.tril(torch.ones((1, seq_len, seq_len), device=device, dtype=torch.bool))
                
                y_pred, cache = self.llm.forward_one_step(lm_input, masks=masks, cache=cache)
                logp = self.llm_decoder(y_pred[:, -1]).log_softmax(dim=-1)
                top_ids = self.sampling_ids(logp.squeeze(dim=0), out_tokens, sampling, ignore_eos=True if i < min_len else False)
                if top_ids in self.stop_token_ids:
                    break
