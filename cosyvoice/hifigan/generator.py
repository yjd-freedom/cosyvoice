...
class SineGen2(torch.nn.Module):
    ...
    def __init__(self, samp_rate, upsample_scale, harmonic_num=0,
        ...
        self.causal = causal
        if causal is True:
            self._init_causal_random_state()
    
    def _init_causal_random_state(self):
        """初始化 Causal 模式下的随机状态"""
    ...
    def refresh_random_state(self):
        """
        刷新随机状态，确保每次新的合成请求都有不同的音频微妙特征。
        在 Causal 模式下，应该在每次新的合成请求开始前调用此方法。
        """
...
class SourceModuleHnNSF(torch.nn.Module):
    ...
    def refresh_random_state(self):
        ...
        if self.causal is True:
            self._init_causal_random_state()
        # 同时刷新子模块的随机状态
        if hasattr(self.l_sin_gen, 'refresh_random_state'):
            self.l_sin_gen.refresh_random_state()
...
class HiFTGenerator(nn.Module):
    ...
    def refresh_random_state(self):
        """
        刷新 HiFiGAN 中的随机状态，确保每次新的合成请求都有不同的音频微妙特征。
        主要影响 Causal 模式下的正弦波生成器和噪声源。
        在每次新的合成请求开始前调用此方法可以避免不同合成请求之间的音频特征相似性。
        """
        if hasattr(self.m_source, 'refresh_random_state'):
            self.m_source.refresh_random_state()
